...
const deleteContextMenu = (event): void => {
  if (
    ...
    event.target.closest(".context-menu__item")
  ) {
...
class Main extends Block {
  ...
  async onDeleteChat(): void {
    if (currentChatId) {
      const result = await chatApi.deleteChat(
  ...
  async onSmallChatClick(event) {
    ...
    //Сокеты:
    const TOKEN_VALUE = await messageApi.getToken(CHAT_ID).then((result) => {
        ...
        <div class="messenger__right">
                    ...
                    <div class="chat__wrapper">
                      <ul class="chat__list-messages">
                          ...
                          .map((message) => {
                            ...
                            const todayFormatDate = `${addZero(
                              today.getDate()
                            )}.${addZero(today.getMonth() + 1)}.${addZero(
                              today.getFullYear()
                            )}`;
                            const formatTime = `${addZero(
                              date.getHours()
                            )}:${addZero(date.getMinutes())}`;
                            const formatDate = `${addZero(
                              date.getDate()
          ...
          <div class="write">
            <form class="${`form form--horizontal${
              this.props.activeChat ? "" : " display-none"
